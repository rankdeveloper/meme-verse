...
import { likeMeme, unlikeMeme } from "../../store/memeSlice";
import { addLikedMeme, removeLikedMeme } from "../../store/userSlice";
...
const MemeCard: React.FC<MemeCardProps> = ({ meme, index = 0 }) => {
  ...
  const handleShare = (e: React.MouseEvent) => {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard
        .writeText(window.location.origin + "/meme/" + meme.id)
  ...
  return (
    <motion.div
      ...
      className={`rounded-lg overflow-hidden shadow-lg hover:shadow-xl hover:scale-110 hover:transition-all  transition-shadow ${
        darkMode ? "bg-[#1c1f26]" : "bg-white border border-purple-500"
      }`}
    >
      <Link to={`/meme/${meme.id}`} className="block">
        ...
        <div className="p-4">
          ...
          <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-3">
            ...
            <span>
              {formatDistanceToNow(new Date(meme.created_at), {
                addSuffix: true,
              })}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span>{meme.likes !== 0 && meme.likes}</span>
              </button>
